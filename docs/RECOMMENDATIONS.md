...
| C5 | N+1 queries on workout-detail fetch | `backend/app/routers/workouts.py:2406` (loop + `db.exec(select(ExerciseSet)…)` per exercise) | Single `IN` query then group by exercise id. |
| C6 | N+1 in social digest builder | `backend/app/services/social/digest.py:92-104` | Batch profile/user/goal lookups. |
| C7 | N+1 in meal item fetch | `backend/app/routers/meals.py:743` (per-meal MealItem query) | Eager-load via `selectinload` or batch. |
...
| C9 | 6 screens import `expo-image-picker` eagerly | ActiveWorkoutScreen, HomeScreen, ProgressScreen, OnboardingScreen, GearScreen, EditProfileScreen | Move all to `await import('expo-image-picker')` (GearScreen already does — pattern exists). |
...
| C11 | bodyScanHistory loaded fully into memory | `src/screens/ProgressScreen.tsx:328` | Paginate or limit to last 20. |
