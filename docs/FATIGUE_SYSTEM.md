# Fatigue System v2.0 — Design & Implementation
...
3. If inferred focus differs from the original label, it's corrected (e.g., prevents "Recovery" label persisting on what was actually a leg day)
...
2. Only focus_label exists -> `resolve_focus_fatigue()` estimates from static mapping
3. Result stored as `resolved_muscle_fatigue` JSON on the completion row
4. `compute_rolling_fatigue()` reads these dicts with time decay to produce current state
...
| File | Purpose |
|------|---------|
| `activity_impact.py` | MuscleFatigue dataclass, resolve functions, rolling fatigue, derived readiness, negative recovery values |
| `history.py` | `get_recent_completions_for_fatigue` DB query |
| `workouts.py` | Complete endpoint (resolve + store), generate-day (graduated response), fatigue endpoint, focus auto-correction |
