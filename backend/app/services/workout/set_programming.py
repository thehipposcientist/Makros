...
def recommend_next_set(
    *,
    ...
    if missed:
        # RIR-aware deload logic. Two orthogonal signals:
        #   - how badly they missed (reps below the bottom)
        #   - whether they had anything left in the tank (actual_rir)
        # Rules:
        #   - Hit failure (rir == 0) and missed by only 1-2 reps → drop
        #     one increment so the next set has a real shot at the range.
        #   - Hit failure AND missed severely (≥ 40% below bottom,
        #     e.g. target 12 → hit 5) → drop proportionally so the next
        #     set lands in range instead of grinding near-failure again.
        #   - Had reps in the tank (rir > 0) → hold load and push for
        #     the range next set. The user wasn't at their limit; what
        ...
        if at_failure and miss_ratio < 0.6 and weight > 0 and inc > 0:
            ...
            # the next set lands inside the range. E.g. target 12, hit 5
            # (ratio 0.42) → drop ~40% of weight rather than -5 lb which
            # would leave the user grinding failure again.
        ...
        # attempt the range again. Only cut load when the user truly
        # went to failure (rir=0).
        return NextSetRecommendation(
            next_set_weight_lbs=weight if weight > 0 else None,
            next_set_rep_target=f"{lo}-{hi}",
            action="hold_load",
            explanation=(
                f"Short of {lo}-{hi} with {int(rir or 0)} RIR — hold load and "
                f"push for the range on the next set."
            ),
        )
