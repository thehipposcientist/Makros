...
def _insert_user(
    session,
    *,
    email: str = "persist_test@example.com",
...
def test_active_workout_endpoint_403_for_free_user() -> None:
    """Free users cannot read generated workout-plan artifacts."""
    print("\n[test] GET /ai/plans/active-workout 403s for free users")
    from fastapi import FastAPI
    from fastapi.testclient import TestClient
    from sqlmodel import Session
    ...
    try:
        ...
        app_db.engine = orig_engine
    _ok("free users are blocked before active-plan lookup")
