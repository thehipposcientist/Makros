...
type RelativeStrengthSource = 'primary' | 'secondary' | 'rolling';

type RelativeStrengthProfile = {
  muscle: string;
  exercise: string;
  score: number;
  ...
  date: string | null;
  sampleCount: number;
  // For secondary credit only: the % of the source lift's estimated 1RM that
  // was attributed to this muscle (so the displayed number is explainable and
  // not mistaken for a direct 1RM).
...
// The targets above are male bodyweight-ratio norms. Women's equivalent
// strength standards (per StrengthLevel / ExRx) are a smaller multiple of
// bodyweight — roughly ~0.62-0.65x for upper body and ~0.72x for lower
// body. Grading a woman against the male table systematically under-scores
// her, so scale the targets down when the profile sex is female. Unknown /
// male sex keeps the original table (no regression for existing users).
...
// Fallback share of a compound's estimated 1RM credited to a *secondary*
// muscle when the lift's primary-mover standard is unknown. When both the
// secondary muscle's and the lift's primary standard are known we scale by
// their ratio instead (see buildRelativeStrengthProfiles) — that keeps credit
// proportional to each muscle's strength standard so a balanced lifter scores
// their secondary muscles in line with the lift, rather than being flagged
// weak by an arbitrary flat factor.
...
function relativeStrengthSetEstimate(set: any, category: LiftCategory): number | null {
...
function buildRelativeStrengthProfiles(
  history: WorkoutSession[],
  ...
  const rawToday = options.today instanceof Date ? dateKey(options.today) : String(options.today ?? '').slice(0, 10);
  const today = /^\d{4}-\d{2}-\d{2}$/.test(rawToday) ? rawToday : dateKey(new Date());
  const start = shiftDateKey(today, -29);
  const byMuscle = new Map<string, RelativeStrengthProfile>();
  ...
  const consider = (candidate: Omit<RelativeStrengthProfile, 'score' | 'ratio'>) => {
    ...
    const rawScore = clampRadarScore((ratio / targetRatio) * 70);
    ...
    const current = byMuscle.get(next.muscle);
    if (!current
      || next.score > current.score
      || (next.score === current.score && next.source === 'primary' && current.source !== 'primary')) {
      byMuscle.set(next.muscle, next);
    }
  };

  for (const session of history) {
    if (!session.completed || session.skipped || !dateInWindow(session.date, start, today)) continue;
    const sessionDate = String(session.date ?? '').slice(0, 10) || null;
    for (const exercise of session.exercises ?? []) {
      ...
      const primary = relativeStrengthPrimaryForName(exercise.name, canonicalRelativeStrengthMuscle(
        ...
        ?? inferChartMuscleFromName(exercise.name),
      ));
      ...
      for (const muscle of secondaries) {
        // Scale the source lift's e1RM by how this muscle's strength standard
        // compares to the lift's primary-mover standard. A balanced lifter then
        // scores a secondary muscle in line with the lift (e.g. a strong squat
        // reads as proportionally strong glutes) instead of being flagged weak
        // by a flat factor. Capped at 1.0 so we never credit a muscle more than
        // the lift itself; `consider` also caps all secondary credit at 82 so
        // indirect estimates can't claim an elite score without direct work.
  ...
  const addRolling = (name: string, lbs: number) => {
    ...
    if (!muscle) return;
    // Rolling/showcase values are all-time bests with no date. They fill
    // muscles you haven't trained directly in the last 30 days, but they
    // must NOT override a recent primary (directly-loaded) set — otherwise
    // an old PR masks what you actually did this week. They may still beat
    // weak secondary credit (e.g. an all-time hip thrust over squat-derived
    // glute credit).
    if (byMuscle.get(muscle)?.source === 'primary') return;
    consider({
      muscle,
      exercise: name,
  ...
  const strengthRadarMetrics = useMemo<RadarMetric[]>(() => {
    ...
    return order.map(item => {
      ...
      const reason = !profile
        ...
        : profile.score >= 80
          ? `${profile.exercise} is strong relative to bodyweight.`
          : profile.score >= 55
            ? `${profile.exercise} is around the expected bodyweight-relative range.`
            : `${profile.exercise} is below the expected bodyweight-relative range.`;
      return {
        key: item.key,
        label: item.label,
